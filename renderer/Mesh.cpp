...
{
    return {
            {{pos.x - scale, pos.y - scale}, {color}, {1.0f, 0.0f}},
            {{pos.x + scale, pos.y - scale}, {color},{0.0f, 0.0f}},
            {{pos.x + scale, pos.y + scale},  {color},{0.0f, 1.0f}},
            {{pos.x - scale, pos.y + scale},  {color},{1.0f, 1.0f}},
    };
...
{
    std::vector<Vertex> new_quad = create_quad_textured(pos,color,scale);
